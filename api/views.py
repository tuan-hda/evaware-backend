...
from helpers.mixins import (
    IncludeDeleteMixin,
    RecombeeProductMixin,
    RecombeeUserMixin,
    RecombeeNetworkError,
...
from .serializers import (
    ...
    ViewCartItemSerializer,
    CartItemSerializer,
    FavoriteItemSerializer,
    ViewFavoriteItemSerializer,
...
class ProductView(IncludeDeleteMixin, ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Product.undeleted_objects.filter(temp["query"])
            .distinct("id", "price")
            .order_by("-id")
        )
...
class ProductDetailAPIView(
    ...
    def update(self, request, *args, **kwargs):
        ...
        if self.set_recombee_item(serializer.instance) == 1:
            ...
            old_serializer = self.get_serializer(
                instance, data=old_data, partial=partial
            )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if not request.user.is_superuser and not request.user.is_staff:
            ...
            items = ListProductSerializer(
                data=recommend_items, many=True, context={"request": request}
            )
            items.is_valid()
...
class RecommendProductsForUserAPIView(ListAPIView, RecombeeRecommendMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = queryset.filter(
            id__in=[recomm["id"] for recomm in recommend["recomms"]]
        )
...
class DeleteRecombeeProductAPIView(GenericAPIView, RecombeeProductMixin):
    ...
    def delete(self, request, id, *args, **kwargs):
        ...
        return Response(
            data={"message": "deleted from Recombee successfully"},
...
class OrderDetailAPIView(RetrieveUpdateAPIView, RecombeeUserMixin):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if request.data["status"] == "Cancelled":
            ...
            self.cancel_order(
                request.user,
                OrderDetail.objects.filter(order_id=instance.id),
...
class ReviewDetailAPIView(
    RetrieveUpdateDestroyAPIView, RecombeeProductMixin, RecombeeUserMixin
):
    """
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if self.set_recombee_item(review.product, review=-review.rating) == 1:
            self.perform_destroy(instance)
            self.delete_rating(
                request.user, review.product, review.created_at.timestamp()
            )
            return Response(status=status.HTTP_204_NO_CONTENT)
        else:
            return self.recombee_network_error()
...
class AddToCartView(GenericAPIView, RecombeeUserMixin):
    ...
    def post(self, request):
        ...
        instance = CartItem.objects.filter(
            created_by=user, variation=variation, product=product
        ).first()
        ...
        if instance is not None:
            ...
            if variation.inventory <= instance.qty:
                ...
                raise serializers.ValidationError(
                    "Insufficient inventory. Update item qty"
                )
...
class MakeOrderFromCartView(GenericAPIView, RecombeeUserMixin):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            self.make_order(
                request.user, cart_items, serializer.instance.created_at.timestamp()
            )
...
class ChangeQtyCartItemAPIView(UpdateAPIView, RecombeeUserMixin):
    ...
    def perform_update(self, serializer):
        ...
        instance.save()
        self.add_cart(
            self.request.user, instance.product, amount=(1 if action == "inc" else -1)
        )
...
class DeleteFavoriteItemView(GenericAPIView, RecombeeUserMixin):
    ...
    def delete(self, request, id, format=None):
        ...
        favorite = FavoriteItem.objects.filter(
            created_by__id=request.user.id, product=product
        )
        if not favorite:
...
class SaleStatisticsAPIView(GenericAPIView):
    ...
    def get(self, request):
        ...
        queryset = Order.objects.filter(
            status="Success", created_at__range=(from_date, to_date)
        )
        prev_queryset = Order.objects.filter(
            status="Success", created_at__range=(prev_from_date, prev_to_date)
        )
...
class TopProductStatisticsAPIView(GenericAPIView):
        ...
        for order in df["order_details"]:
            ...
            for order_detail in order:
                ...
                if product_id in top_products:
                    ...
                    top_products[product_id] = {"sales": qty, "revenue": float(total)}

        top_products_df = pd.DataFrame.from_dict(top_products, orient="index")
        top_products_df = top_products_df.sort_values(
            by=["revenue", "sales"], ascending=[False, False]
        )
        res = (
            top_products_df.reset_index()
            .rename(columns={"index": "product"})
            .to_dict(orient="records")
        )
        for item in res:
            item["product"] = ListProductSerializer(
                instance=Product.objects.get(id=item["product"]),
    ...
    def get(self, request):
        ...
        if range_type == "monthly":
            queryset = Order.objects.filter(
                status="Success",
                created_at__month=current_time_date.month,
            ...
            queryset = Order.objects.filter(
                status="Success",
                created_at__year=current_time_date.year,
                created_at__lte=current_time,
            )
        else:
            ...
            queryset = Order.objects.filter(
                status="Success",
                created_at__month__gte=from_month,
                created_at__lte=current_time,
            )
        ...
        for order in df["order_details"]:
            for order_detail in order:
                ...
                if category_id in category:
                    ...
                    category[category_id] = qty
                sum_qty += qty
        top_categories_df = pd.DataFrame.from_dict(
            category, orient="index", columns=["qty"]
        )
        top_categories_df = top_categories_df.sort_values(by=["qty"], ascending=[False])
        arr = (
            top_categories_df.reset_index()
            .rename(columns={"index": "category"})
            .to_dict(orient="records")
        )
        for obj in arr:
            obj["percentage"] = round(obj["qty"] * 100.0 / sum_qty)
            obj["category"] = CategorySerializer(
                instance=Category.objects.get(id=obj["category"])
            ).data
...
class GetProductFilter(IncludeDeleteMixin, GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        width = (
            queryset.order_by("width").values_list("width", flat=True).distinct("width")
        )
        height = (
            queryset.order_by("height")
            .values_list("height", flat=True)
            .distinct("height")
        )
        length = (
            queryset.order_by("length")
            .values_list("length", flat=True)
            .distinct("length")
        )
        weight = (
            queryset.order_by("weight")
            .values_list("weight", flat=True)
            .distinct("weight")
        )
        material = (
            queryset.order_by("material")
        ...
        variations = (
            variations.filter(product__in=queryset)
            .order_by("name")
...
class SuggestVoucher(GenericAPIView):
    def get(self, request, *args, **kwargs):
        ...
        serializer = ListProductSerializer(
            data=query_set, many=True, context={"request": request}
        )
        serializer.is_valid()
